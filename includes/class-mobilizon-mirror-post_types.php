 ...
 */
class Mobilizon_Mirror_Post_Types {
	...
	private function register_single_post_type( $fields ) {
		...
		$labels = array(
			...
			// 'new_item'              => sprintf( __( 'New %s', 'mobilizon-mirror' ), $fields['singular'] ),
			// 'add_new_item'          => sprintf( __( 'Add new %s', 'mobilizon-mirror' ), $fields['singular'] ),
			// 'edit_item'             => sprintf( __( 'Edit %s', 'mobilizon-mirror' ), $fields['singular'] ),
			// 'view_item'             => sprintf( __( 'View %s', 'mobilizon-mirror' ), $fields['singular'] ),
			// 'view_items'            => sprintf( __( 'View %s', 'mobilizon-mirror' ), $fields['plural'] ),
			// 'search_items'          => sprintf( __( 'Search %s', 'mobilizon-mirror' ), $fields['plural'] ),
			// 'not_found'             => sprintf( __( 'No %s found', 'mobilizon-mirror' ), strtolower( $fields['plural'] ) ),
			// 'not_found_in_trash'    => sprintf( __( 'No %s found in trash', 'mobilizon-mirror' ), strtolower( $fields['plural'] ) ),
			// 'all_items'             => sprintf( __( 'All %s', 'mobilizon-mirror' ), $fields['plural'] ),
			// 'archives'              => sprintf( __( '%s', 'mobilizon-mirror' ), $fields['singular'] ),
			// 'attributes'            => sprintf( __( '%s Attributes', 'mobilizon-mirror' ), $fields['singular'] ),
			// 'insert_into_item'      => sprintf( __( 'Insert into %s', 'mobilizon-mirror' ), strtolower( $fields['singular'] ) ),
			// 'uploaded_to_this_item' => sprintf( __( 'Uploaded to this %s', 'mobilizon-mirror' ), strtolower( $fields['singular'] ) ),

			/* Labels for hierarchical post types only. */
			// 'parent_item'           => sprintf( __( 'Parent %s', 'mobilizon-mirror' ), $fields['singular'] ),
			// 'parent_item_colon'     => sprintf( __( 'Parent %s:', 'mobilizon-mirror' ), $fields['singular'] ),
	...
	private function register_single_post_type_taxonomy( $tax_fields ) {
		...
		$labels = array(
			...
			// 'all_items'                  => sprintf( __( 'All %s' , 'mobilizon-mirror' ), $tax_fields['plural'] ),
			// 'edit_item'                  => sprintf( __( 'Edit %s' , 'mobilizon-mirror' ), $tax_fields['single'] ),
			// 'view_item'                  => sprintf( __( 'View %s' , 'mobilizon-mirror' ), $tax_fields['single'] ),
			// 'update_item'                => sprintf( __( 'Update %s' , 'mobilizon-mirror' ), $tax_fields['single'] ),
			// 'add_new_item'               => sprintf( __( 'Add New %s' , 'mobilizon-mirror' ), $tax_fields['single'] ),
			// 'new_item_name'              => sprintf( __( 'New %s Name' , 'mobilizon-mirror' ), $tax_fields['single'] ),
			// 'parent_item'                => sprintf( __( 'Parent %s' , 'mobilizon-mirror' ), $tax_fields['single'] ),
			// 'parent_item_colon'          => sprintf( __( 'Parent %s:' , 'mobilizon-mirror' ), $tax_fields['single'] ),
			// 'search_items'               => sprintf( __( 'Search %s' , 'mobilizon-mirror' ), $tax_fields['plural'] ),
			// 'popular_items'              => sprintf( __( 'Popular %s' , 'mobilizon-mirror' ), $tax_fields['plural'] ),
			// 'separate_items_with_commas' => sprintf( __( 'Separate %s with commas' , 'mobilizon-mirror' ), $tax_fields['plural'] ),
			// 'add_or_remove_items'        => sprintf( __( 'Add or remove %s' , 'mobilizon-mirror' ), $tax_fields['plural'] ),
			// 'choose_from_most_used'      => sprintf( __( 'Choose from the most used %s' , 'mobilizon-mirror' ), $tax_fields['plural'] ),
			// 'not_found'                  => sprintf( __( 'No %s found' , 'mobilizon-mirror' ), $tax_fields['plural'] ),
		);
	...
	public function create_custom_post_type() {
		...
		$post_types_fields = array(
			...
			array(
				 ...
				 */
				'slug'                  => 'mobilizon_event',

				'singular'              => 'Mobilizon Event',
				'plural'                => 'Mobilizon Events',
				'menu_name'             => 'Events',
				...
				'rewrite' => array(
					...
					'slug'       => 'events', // string (defaults to the post type name)
				...
				//'capability_type'       => 'mobilizon_event',
				...
				'taxonomies'            => array(
                    ...
                    array(
                        'taxonomy'              => 'mobilizon_group',
                        'plural'                => 'Mobilizon Groups',
                        'single'                => 'Mobilizon Group',
                        'post_types'            => array( 'mobilizon_event' ),
                    ),
		...
		//register_taxonomy_for_object_type( 'post_tag', 'mobilizon_event');

	}

	public function add_mobilizon_to_tag_archive( $query ) {
		if ( $query->is_tag() && $query->is_main_query() ) {
			$query->set( 'post_type', array( 'post', 'mobilizon_event' ) );
		}
	}
